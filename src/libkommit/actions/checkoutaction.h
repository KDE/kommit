...
#include <Kommit/AbstractAction>
#include <Kommit/FileDelta>
#include <Kommit/FileMode>
#include <Kommit/Object>
...
class Repository;
class CheckoutAction : public AbstractAction
{
    Q_OBJECT
...
public:
    ...
    CheckoutAction(Repository *repo, QObject *parent = nullptr);

    Object target() const;
    void setTarget(const Object &target);

    QString ancestorLabel() const;
    void setAncestorLabel(const QString &ancestorLabel);

    QString ourLabel() const;
    void setOurLabel(const QString &ourLabel);

    QString theirLabel() const;
    void setTheirLabel(const QString &theirLabel);
    ...
    void setNotifyFlags(const NotifyFlags &notifyFlags);

    CheckoutStrategies checkoutStrategies() const;
    void setCheckoutStrategies(const CheckoutStrategies &checkoutStrategies);
...
private:
    QSharedPointer<CheckoutPrivate> d;
};
